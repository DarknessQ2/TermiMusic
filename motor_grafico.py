...
# =====================================================================
# 🛠️ CONFIGURACIÓN DE COLOR POR DEFECTO
# =====================================================================
COLOR_DEFECTO = (0, 255, 255)  # Color Cyan de respaldo si el modo está apagado o falla
# =====================================================================

arte_cache = None
ruta_actual = ""
DISCO_ANIM = []
COLOR_CARATULA = COLOR_DEFECTO
...
def renderizar_imagen(fuente):
    ...
    try:
        ...
        # 🔄 CONECTADO: Extracción del color promedio según el estado dinámico global
        if cfg.Estado.modo_dinamico:
            img_min = img.resize((1, 1), Image.Resampling.LANCZOS)
            COLOR_CARATULA = img_min.getpixel((0, 0))
        else:
            COLOR_CARATULA = COLOR_DEFECTO

        img = img.resize((48,26), Image.Resampling.LANCZOS)
        lineas = []
...
def render_menu_vinilos(playlists_web, playlists_locales, seleccion_idx, agarrado, frame_caida):
    ...
    if cfg.Estado.modo_dinamico:
        r, g, b = COLOR_CARATULA
        ansi_color = f"\033[38;2;{r};{g};{b}m"
        ansi_color_b = f"\033[38;2;{r};{g};{b};1m"
        ansi_fondo = f"\033[48;2;{r};{g};{b}m\033[1;30m"
    else:
        ansi_color = "\033[1;36m"
        ansi_color_b = "\033[1;36m"
        ansi_fondo = "\033[7;37m"

    pintar(2, 4, "=== TIENDA DE VINILOS (TERMI-MUSIC) ===", ansi_color_b)
    ...
    for i in range(inicio, fin):
        ...
        if i == seleccion_idx:
            pintar(fila_actual, caja_c, f" │ > [{etiqueta}] {nombre[:18]:<18} │", ansi_color_b)
        else:
            color_dim = "\033[38;5;13m" if es_web else "\033[38;5;14m"
            pintar(fila_actual, caja_c, f" │ \033[0m{color_dim}  [{etiqueta}] {nombre[:18]:<18} \033[0m{color_caja}│", color_caja)

    pintar(caja_r + 2 + (fin - inicio), caja_c, " └──────────────────────────┘", color_caja)
    ...
    if agarrado:
        ...
        for j, linea in enumerate(DISCO_DETALLADO):
            pintar(r_anim + j, c_anim, linea, color_disco)
        pintar(r_anim + 9, c_anim + 19, f"[{etiq_sel}]", "\033[1;30;47m")
        ...
        pintar(visor_r + 21, visor_c + 12, f" ALBUM: {nom_sel[:18]:<18} ", ansi_fondo)
